...
where
    TParams: IntoParams + CondSend + Sync + 'static,
    TResponse: Serialize + DeserializeOwned + CondSend + Sync,
...
where
    TCreate: Serialize + Sync + CondSend,
...
where
    TCreate: Serialize + Sync + CondSend + EqIdentity + 'a + Clone + IntoPatch<TUpdate>,
    TUpdate: Serialize + Sync + CondSend + Default,
    TResponse: Serialize + DeserializeOwned + Sync + CondSend,
    Self: WithApiClient + WithBasePath + Sync,
{
...
pub trait UpsertCollection<TUpsert, TResponse> {
    ...
    where
        TUpsert: Serialize + Sync + CondSend,
        TResponse: Serialize + DeserializeOwned + Sync + CondSend,
        Self: WithApiClient + WithBasePath + Sync,
    {
        async move {
...
where
    TUpdate: Serialize + Sync + CondSend,
...
where
    TRequest: Serialize + Sync + CondSend,
    TResponse: Serialize + DeserializeOwned,
    Self: WithApiClient + WithBasePath + Sync,
{
...
where
    TFilter: Serialize + Sync + CondSend + 'static,
    TResponse: Serialize + DeserializeOwned,
    Self: WithApiClient + WithBasePath + Sync,
{
...
where
    TFilter: Serialize + Sync + CondSend + 'a,
    TSearch: Serialize + Sync + CondSend + 'a,
    TResponse: Serialize + DeserializeOwned,
    Self: WithApiClient + WithBasePath + Sync,
{
