use cognite::{time_series::TimeSeries, Identity};
use std::collections::HashMap;

#[derive(Default)]
pub struct TimeSeriesStore {
    pub by_id: HashMap<i64, TimeSeries>,
    pub by_ext: HashMap<String, i64>,
    pub order: Vec<i64>,
}

impl TimeSeriesStore {
    pub fn insert(&mut self, ts: TimeSeries) {
        let id = ts.id;
        if let Some(ext) = &ts.external_id {
    ...
    pub fn remove(&mut self, id: i64) -> Option<TimeSeries> {
    ...
    pub fn get_by_identity(&self, identity: &Identity) -> Option<&TimeSeries> {
        match identity {
    ...
    pub fn get_mut_by_identity(&mut self, identity: &Identity) -> Option<&mut TimeSeries> {
        match identity {
    ...
    pub fn filter<F: Fn(&TimeSeries) -> bool>(&self, f: F) -> Vec<TimeSeries> {
        self.order
            .iter()
