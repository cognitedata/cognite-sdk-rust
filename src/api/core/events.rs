...
impl Events {
    ...
    pub async fn create(&self, events: &[Event]) -> Result<Vec<Event>> {
        ...
        let events_response: EventListResponse =
            self.api_client.post("events", &event_items).await?;
        Ok(events_response.items)
    }

    pub async fn filter_all(&self, event_filter: EventFilter) -> Result<Vec<Event>> {
        let filter: Filter = Filter::new(event_filter, None, None);
        let events_response: EventListResponse =
            self.api_client.post("events/list", &filter).await?;
        Ok(events_response.items)
    }
    ...
    pub async fn retrieve(&self, event_ids: &[u64]) -> Result<Vec<Event>> {
        let id_list: Vec<EventId> = event_ids.iter().copied().map(EventId::from).collect();
        let event_items = Items::from(&id_list);
        let events_response: EventListResponse =
            self.api_client.post("events/byids", &event_items).await?;
        Ok(events_response.items)
    }

    pub async fn update(&self, events: &[Event]) -> Result<Vec<Event>> {
        let patch_events: Vec<PatchEvent> = events.iter().map(PatchEvent::from).collect();
        let event_items = Items::from(&patch_events);
        let events_response: EventListResponse =
            self.api_client.post("events/update", &event_items).await?;
        Ok(events_response.items)
    }
    ...
    pub async fn delete(&self, event_ids: &[u64]) -> Result<()> {
        let id_list: Vec<EventId> = event_ids.iter().copied().map(EventId::from).collect();
        let event_items = Items::from(&id_list);
        self.api_client
