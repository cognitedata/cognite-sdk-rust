...
impl ServiceAccounts {
  ...
  pub fn list_all(&self, params : Option<Vec<Params>>) -> Result<Vec<ServiceAccount>> {
    let service_accounts_response : ServiceAccountListResponse = self.api_client.get_with_params("serviceaccounts", params)?;
  ...
  pub fn create(&self, service_accounts : &[ServiceAccount]) -> Result<Vec<ServiceAccount>> {
    ...
    let service_accounts_response : ServiceAccountListResponse = self.api_client.post("serviceaccounts", &service_accounts_items)?;
    Ok(service_accounts_response.items)
  }

  pub fn delete(&self, service_account_ids : &[u64]) -> Result<()> {
    let id_items = Items::from(service_account_ids);
    self.api_client.post::<::serde_json::Value, Items>("serviceaccounts/delete", &id_items)?;
    Ok(())
  }
}