...
use crate::{time_series::*, Identity};
    ...
    fn from(req: AddDatapoints) -> DataPointInsertionItem {
        DataPointInsertionItem {
            ...
            datapoint_type: match req.datapoints {
                DatapointsEnumType::NumericDatapoints(dps) => {
                    Some(DatapointType::NumericDatapoints(NumericDatapoints {
                        datapoints: dps.into_iter().map(NumericDatapoint::from).collect(),
                    }))
                }
                DatapointsEnumType::StringDatapoints(dps) => {
                    Some(DatapointType::StringDatapoints(StringDatapoints {
